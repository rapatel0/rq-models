...
| **Phase 2c full serve** (end-to-end token generation with rotorquant pack/unpack on hot path) | ✅ 2026-04-26 | Generates with HTTP 200 on Qwen/Qwen3-4B (bf16) via `rq-vllm:latest` after two fixes (rq-vllm `a67d04eb2`, overlay `1fe8eb6`): (1) `_round_trip_planar3` now casts bf16→fp16 for the kernel call and back, (2) `kv_cache_dtype_str_to_dtype` routes `rotorquant_*` through the model-dtype branch so cache + K/V dtypes match. Sample completion: prompt `"The capital of France is"` → `" the city of the... the city of the..."` — engine stable, output quality degraded vs the fp16 baseline (`" Paris. The capital of Germany is Berlin..."`) which is the Phase 3 PPL-gate measurement target, not a serve-path blocker. Host prerequisite: flaky P-cores must be offline (`sudo chcpu -d 1,2,6,8,9,10,11,12,16,25,29`) — with them online the unmodified `vllm/vllm-openai:v0.19.1` image also SIGSEGVs with the bytecode-corruption opcode-184 pattern. |
...
| **Phase 3**: Δppl ≤ 0.05% gate vs llama.cpp planar3 baseline | ⏸ pending — serve-path now clear | hard gate. Run `scripts/eval_perplexity.py` against the integrated stack. The Qwen3-4B smoke output above suggests Phase 2c integration drops more quality than Phase 2a synthetic tests predicted (cos-sim 0.97 / p99 normalized err 0.16), so investigate before declaring 0.05% achievable: candidates are RoPE-after-quantize ordering, K/V layout vs llama.cpp byte-for-byte parity, and codebook fit for post-QKnorm K distributions. |
