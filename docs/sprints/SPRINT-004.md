...
| **Phase 3**: Δppl ≤ 0.05% gate vs llama.cpp planar3 baseline | ⏸ pending llama.cpp run | hard gate is cross-substrate parity (rq-vllm vs llama.cpp planar3). vLLM-internal Δppl(rq3 − fp16) on Qwen3.5-4B over a 698-token corpus is **+1.02 %** (236-token earlier run was −1.14 %; sign flip = inside noise floor at this corpus size). 11 of 13 paragraphs within ±2 %; two outliers at +6.8 % and +8.7 % on date/numeric-fact recall (`artifacts/SPRINT-004-PHASE3-PPL.md`). The scorer (`scripts/eval_perplexity.py`) hits any OpenAI-compatible endpoint, so llama.cpp planar3 server + same corpus is the next step for the actual gate measurement. |
