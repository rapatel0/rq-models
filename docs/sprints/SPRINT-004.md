...
| **Phase 2b**: wire kernels into `setup.py` build + replace `rotorquant_kv.py` passthrough stubs | ⏸ next | one-line pyproject extension entry + ~10 lines in `rotorquant_kv.py` to dispatch into `torch.ops.rq_models.rotorquant_planar3_*`. |
| **Phase 2c**: integrate into FlashAttention `reshape_and_cache_flash` call site | ⏸ pending 2b | replaces the upstream f16 KV-write op with the rotorquant pack op when `kv_cache_dtype.startswith("rotorquant_")`. |
