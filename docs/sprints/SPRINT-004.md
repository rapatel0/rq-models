...
| **Cross-substrate parity harness** (planar3 CUDA vs llama.cpp-turboquant C reference) | ✅ 2026-04-27 | Adds `scripts/cross_substrate_parity.py` (capture K → run our kernel → save fp16 blob) and `scripts/cross_substrate_ref/{rq_planar3_ref.c,README.md}` (companion C driver that runs the same input through llama.cpp-turboquant's `quantize_row_planar3_reference` + `dequantize_row_planar3`). Both substrates write the same binary protocol (`RQK1` magic + uint32 n_blocks + fp16 payload) so the diff stage is byte-exact. The harness is the strongest "no-bug" check we can run for the rq-vllm port: it catches divergences against the reference C kernel on any input distribution including pathological ones (Qwen3 L0 k_norm), where synthetic-Gaussian parity tests have no opinion. Build instructions inline; expected outcome on a clean port is **bit-identical 100 %, 0 ULP diff** (or ≤ 2 ULP per element, acceptable for fp-arithmetic reordering between CUDA / CPU). |
