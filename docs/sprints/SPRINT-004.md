...
| **Phase 3**: Δppl ≤ 0.05% gate vs llama.cpp planar3 baseline | ⏸ pending llama.cpp run | hard gate is cross-substrate parity (rq-vllm vs llama.cpp planar3). vLLM-internal Δppl(rq3 - fp16) on Qwen3.5-4B is **−1.14 %** over a 236-token corpus (artifacts/SPRINT-004-PHASE3-PPL.md), ±5 % per-paragraph — statistical parity at this corpus size. `scripts/eval_perplexity.py` can point at any OpenAI-compatible endpoint, so llama.cpp planar3 server + same corpus is the next step for the actual gate measurement. |
