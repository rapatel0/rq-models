...
| **Phase 3**: Δppl ≤ 0.05% gate vs llama.cpp planar3 baseline | ⏸ pending — gate now scoped to Qwen3.5/3.6 | hard gate. Run `scripts/eval_perplexity.py` against the integrated stack on a Qwen3.5 model (smoke output is essentially baseline-quality so 0.05% is plausible). Qwen3 and earlier-family models with k_norm γ blow-up are explicitly excluded — see the quality-probe row above and `artifacts/SPRINT-004-PHASE2C-QUALITY-PROBE.md`. |
