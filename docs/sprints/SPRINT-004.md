...
| **Phase 2c full serve** (end-to-end token generation with rotorquant pack/unpack on hot path) | ⏸ blocked by host instability | Three runs all hit the generic Python 3.12 SIGSEGV after `Model loading took 7.56 GiB memory` — the same non-deterministic stdlib crash that affects unmodified vLLM v0.19.1 on this host. CPython internal trace (no rotorquant in the path), pure environmental. Rerun on RTX 5090 / stable kernel. |
...
| **Phase 3**: Δppl ≤ 0.05% gate vs llama.cpp planar3 baseline | ⏸ pending 2c serve | hard gate. Run `scripts/eval_perplexity.py` against the integrated stack. |
